...
#define MARKER_LENGTH 10   // number of markers in the song at which to change states
...
void setup()
{
  ...
  machine.markers[5] = {5000,    false,          false,      false,     false,     true,  false,  false,     true,      "                ", "                "};
  machine.markers[6] = {10000,   false,          false,      false,     false,     false, true,   true,      false,      "   Scanning     ", "quantum field..."};
  machine.markers[7] = {16000,   false,          false,      false,     false,     true,  false,  true,      false,      "     Source     ", "  responding... "};
  machine.markers[8] = {19000,   false,          false,      false,     false,     false, false,  true,      false,       "   Dispensing   ", "      joy...    "};
  machine.markers[9] = {40000,   false,          true,       false,     true,      false, true,   false,      true,       "                ", "                "};
  machine.isOn = false;
  machine.elapsed = 0;
  machine.startedAt = 0;
...
void initAudio()
{
  ...
  audio.connecttoFS(SD, player.fileToPlay);
  Serial.println("connected to audio");
  Serial.print("track: ");
  Serial.println(player.fileToPlay);
}
...
void loop_TestPlayer()
{
  if (encoder.pressed)
  {
    ...
    if (player.isLooping)
    {
      Serial.println("audio.connecttoFS");
      initAudio();
    }
    else
    {
      Serial.println("audio.stopSong");
      audio.stopSong();
    }
  }
  ...
  if (machine.isOn)
  {
    ...
    for (i = 0; i < MARKER_LENGTH; i++ )
    {
      ...
      if (machine.elapsed >= machine.markers[i].startAt)
      {
        ...
        {
          Serial.printf("player.isLooping, marker %d, elapsed %d\n", i, machine.elapsed);
          initAudio();
        ...
        // instruction done! set marker.markerComplete
    ...
    // pump out audio every cycle
    if (player.isLooping) audio.loop();
  }
}
